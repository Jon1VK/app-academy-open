...
def xnor_select(arr, prc_1, prc_2)
    arr.select do |el|
        res_1, res_2 = prc_1.call(el), prc_2.call(el)
...
def selected_map!(arr, prc_1, prc_2)
    (0...arr.length).each do |i|
        arr[i] = prc_2.call(arr[i]) if prc_1.call(arr[i])
    end
    nil
end
...
# chain_map
# Write a method that accepts any value and an array of procs as an argument. The method should return the final result of feeding the value through all of the procs.

def chain_map(value, procs)
    procs.inject(value) { |result, prc| prc.call(result) }
end
...
def procipher(sentence, hash)
    sentence.split.map do |word|
        ...
        hash.each do |key_prc, value_prc|
            new_word = value_prc.call(new_word) if key_prc.call(word)
        end
        new_word
    end.join(' ')
end
