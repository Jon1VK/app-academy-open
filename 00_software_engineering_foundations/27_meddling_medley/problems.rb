...
def duos(str)
    (1...str.length).count { |i| str[i-1] == str[i] }
end
...
def hash_mapped(hash, prc, &block)
    hash.map { |k, v| [block.call(k), prc.call(v)] }.to_h
end
...
def triplet_true?(str)
    (2...str.length).any? { |i| str[i-2] == str[i-1] && str[i-1] == str[i] }
end
...
def convert_pig_latin(sentence)
    ...
    sentence.split.map do |word|
        capitalized_word = word.capitalize == word
        ...
        if word.length < 3
            ...
            word.capitalize! if capitalized_word
            word + 'ay'
        end
    end.join(' ')
end
    ...
    sentence.split.map do |word|
        ...
        if word.length >= 3
            if vowels.include?(word[-1])
                ...
                i = word.length - 1
                i -= 1 while !vowels.include?(word[i])
                word += word[i..-1]
            end
        end
...
def alternating_vowel(sentence)
    ...
    sentence.split.map.with_index do |word, i|
        if i.even?
            j = 0
            j += 1 while j < word.length && !vowels.include?(word[j])
            word[j] = '' if j < word.length
        else
            j = word.length - 1
            j -= 1 while j >= 0 && !vowels.include?(word[j])
            word[j] = '' if j >= 0
        end
        word
    end.join(' ')
